...
const currentUser = {
    ...
    first_name:
        document.querySelector('meta[name="data-user-first-name"]')?.content ||
        "",
    last_name:
        document.querySelector('meta[name="data-user-last-name"]')?.content ||
        "",
    username:
        document.querySelector('meta[name="data-user-username"]')?.content ||
        "",
    profile_photo_url:
        document.querySelector('meta[name="data-user-photo"]')?.content || "",
};
...
function renderUserUI() {
    ...
    const photoUrl =
        profileData.profile_photo_url || currentUser.profile_photo_url;
    const fullName =
    ...
    setText(
        "statResourceCount",
        fmtNum(profileData.stats?.resources_uploaded ?? 0),
    );
    setText(
        "statStudySessions",
    ...
    setText(
        "statFocusTime",
        fmtFocus(profileData.stats?.total_focus_seconds ?? 0),
    );
...
async function loadProfilePosts() {
    ...
    if (!feed || !currentUser.id) {
        if (feed)
            feed.innerHTML = `<div class="error-state">❌ Not logged in.</div>`;
        return;
    }
    try {
...
function postCardHTML(post, isOwn = false) {
    ...
    const name =
        `${author.first_name || currentUser.first_name || ""} ${author.last_name || currentUser.last_name || ""}`.trim() ||
        "You";
    const photo =
        author.profile_photo_url || currentUser.profile_photo_url || "";
    const initials = mkInitials(
        author.first_name || currentUser.first_name,
        author.last_name || currentUser.last_name,
    );
    ...
    if (media.length) {
        mediaHTML =
            `<div class="post-media-grid count-${Math.min(media.length, 4)}">` +
            media
                .slice(0, 4)
                .map((url, i) => {
                    ...
                    const more =
                        i === 3 && media.length > 4
                            ? `<div class="media-more">+${media.length - 4}</div>`
                            : "";
        ...
        mediaHTML +=
            ...
            files
                .map(
                    (
                        f,
                    ) => `<a class="post-file-chip" href="${escH(f.url)}" target="_blank" download>
                📎 <span>${escH(f.name || "File")}</span></a>`,
                )
                .join("") +
            `</div>`;
    }
...
async function loadFollowCounts() {
    ...
    try {
        const [fersRes, fingRes] = await Promise.all([
            sbFetch(
                `${SB_URL}/rest/v1/follows?following_id=eq.${currentUser.id}&select=id`,
            ),
            sbFetch(
                `${SB_URL}/rest/v1/follows?follower_id=eq.${currentUser.id}&select=id`,
            ),
        ]);
...
async function openFollowModal(mode) {
    ...
    try {
        ...
        const res = await sbFetch(
            `${SB_URL}/rest/v1/follows?${filter}&select=${fkCol},profiles:${fkCol}(id,first_name,last_name,username,profile_photo_url)`,
        );
        const rows = await res.json();
        if (!rows?.length) {
            list.innerHTML =
                '<div style="padding:20px;text-align:center;color:#9ca3af;font-size:14px;">Nobody here yet.</div>';
            return;
        }
        list.innerHTML = rows
            .map((r) => {
                ...
                <div class="follow-modal-avatar">${
                    p.profile_photo_url
                        ? `<img src="${escH(p.profile_photo_url)}" alt="">`
                        : ini
                }</div>
                <div>
...
function openEditModal(postId) {
    const text =
        document.querySelector(`#post-${postId} .post-text`)?.textContent || "";
...
async function saveEdit() {
    ...
    try {
        const res = await fetch(
            `${SB_URL}/rest/v1/posts?id=eq.${editingPostId}`,
            {
                method: "PATCH",
                headers: {
                    apikey: SB_SVC,
...
function handleProfilePhotoChange(event) {
    ...
    reader.onload = () => {
        ...
        ["profileAvatarLarge", "sidebarAvatar", "topBarAvatar"].forEach(
            (id) => {
...
function likePost(id) {
    window.location.href = `/newsfeed#post-${id}`;
}
...
function setAvatar(id, photoUrl, name, initials) {
    ...
    el.innerHTML = photoUrl
        ? `<img src="${escH(photoUrl)}" alt="${escH(name)}">`
        : initials;
}
