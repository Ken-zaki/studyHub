    ...
    async function determineAdminStatus() {
        ...
        try {
            const r = await fetch(`/study-groups/${activeGroupId}/members`);
            if (!r.ok) return;
        ...
        messages.forEach(m => {
            ...
            html += `<div class="msg-row ${isOwn ? 'own' : ''}" id="msg-${msgIdSafe}"
                ...
                <div class="msg-actions">
                    <button class="msg-action-btn" data-msg-id="${msgIdSafe}" onclick="openThreadById(this)" title="Reply in thread">💬</button>
                    ${deleteBtn}
                </div>
            </div>`;
        });

        box.innerHTML = html || '<div class="no-msg" style="margin-top:20px;">No messages yet. Say hello! 👋</div>';
