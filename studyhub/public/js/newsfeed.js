...
async function toggleFollow(userId, btn) {
    ...
    try {
        if (isFollowing) {
            // UNFOLLOW — delete, no notification
            await _sb
                .from("follows")
                .delete()
                .eq("follower_id", currentUser.id)
            ...
            // Fire notification in background — no await
            const senderName =
            ...
            insertNotification({
                ...
                trigger: "followed_by_" + currentUser.id,
...
async function loadFeed() {
    ...
    try {
        ...
        if (currentTab === "for_you") {
            ...
            const { data: publicPosts, error: pubErr } = await _sb
                .from("posts")
                .select(
            ...
            if (friendSet.size) {
                const { data: friendPosts } = await _sb
                    .from("posts")
                    .select(
                        "*, profiles(id, first_name, last_name, username, profile_photo_url)",
                    )
            ...
            const { data: followedPosts, error: folErr } = await _sb
                .from("posts")
                .select(
                    "*, profiles(id, first_name, last_name, username, profile_photo_url)",
                )
            ...
            const { data: friendsPosts, error: frErr } = await _sb
                .from("posts")
                .select(
                    "*, profiles(id, first_name, last_name, username, profile_photo_url)",
                )
...
function postCardHTML(post, likeCount, isLiked) {
    ...
    const visIcon =
        { public: "🌐", friends: "👥", only_me: "🔒" }[post.visibility] || "🌐";
    ...
    const files = post.file_urls
        ? Array.isArray(post.file_urls)
            ? post.file_urls
            : JSON.parse(post.file_urls)
        : [];
    const linkMeta = post.link_meta
        ? typeof post.link_meta === "object"
            ? post.link_meta
            : JSON.parse(post.link_meta)
        : null;
    ...
    <div class="post-card" id="post-${post.id}">
        ...
        ${
            text || media.length || files.length || linkMeta
                ? `<div class="post-body">
            ${textHTML}
        </div>`
                : ""
...
async function toggleLike(postId, btn) {
    ...
    try {
            ...
            _sb.from("posts")
                ...
                .then(({ data: postData }) => {
                    ...
                    insertNotification({
                        ...
                        source_type: "post",
                        source_id: postId,
                        trigger: "post_liked_" + currentUser.id,
                        title: `❤️ ${senderName} liked your post`,
                        icon: "❤️",
                        urgency: "info",
                    });
                });
        ...
        const countEl = document.querySelector(
            `#post-${postId} .post-counts-likes`,
        );

        if (countEl)
            countEl.innerHTML =
                count > 0 ? `❤️ ${count} like${count !== 1 ? "s" : ""}` : "";
    } catch (e) {
        // Revert on error
        btn.classList.toggle("liked", isLiked);
    }
...
async function loadComments(postId) {
    ...
    try {
        ...
        const total =
            (data?.length || 0) + Object.values(repliesMap).flat().length;
        const countEl = document.querySelector(
            `#post-${postId} .post-counts-comments`,
        );
        if (countEl)
        ...
        list.innerHTML = data
            .map((c) => commentHTML(c, repliesMap[c.id] || []))
            .join("");
    } catch (e) {
        list.innerHTML =
...
async function submitComment() {
    ...
    try {
        ...
        // Clear input and reload immediately — don't wait for notification
        input.value = "";
        input.style.height = "";
        loadComments(activePostId);
        ...
        _sb.from("posts")
            ...
            .then(({ data: postData }) => {
                ...
                insertNotification({
                    user_id: postData?.user_id,
                    source_type: "post",
                    source_id: activePostId,
                    trigger:
                        "post_commented_" + currentUser.id + "_" + Date.now(),
...
async function createPost() {
    ...
    try {
        ...
        const recipientIds = new Set([...followingSet, ...friendSet]); // followingSet = people YOU follow; we need followers instead
        // Fetch who follows currentUser
        const { data: followerData } = await _sb
            .from("follows")
            .select("follower_id")
            .eq("following_id", currentUser.id);
        const { data: friendData } = await _sb
            .from("friendships")
            .select("user_id, friend_id")
            .or(`user_id.eq.${currentUser.id},friend_id.eq.${currentUser.id}`);

        const notifyIds = new Set();
        (followerData || []).forEach((r) => notifyIds.add(r.follower_id));
        (friendData || []).forEach((r) => {
            const other =
        ...
        for (const uid of notifyIds) {
            await insertNotification({
                ...
                trigger: "new_post_" + newPost?.id,
                title: `📝 ${senderName} made a new post`,
                body: postPreview,
                icon: "📝",
                urgency: "info",
            });
...
async function saveEdit() {
    ...
    try {
        const { error } = await _sb
            .from("posts")
            .update({ content: content || null, visibility: vis })
            .eq("id", editingPostId);
        if (error) throw error;
        closeEditModal();
        loadFeed();
...
async function loadWhoToFollow() {
    ...
    try {
        ...
        if (!suggestions.length) {
            el.innerHTML =
                '<div class="res-empty-small">Nothing to suggest right now.</div>';
            return;
        }
...
async function insertNotification(row) {
    if (!row.user_id || row.user_id === currentUser.id) return; // never notify yourself
    try {
