...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (!e.target.closest(".sort-filter-wrap"))
            document
                .querySelector(".sort-filter-menu")
                ?.classList.remove("open");
        if (!e.target.closest(".post-menu-wrap"))
            document
                .querySelectorAll(".post-menu-dropdown.open")
...
async function toggleFollow(userId, btn) {
    ...
    try {
        ...
        document
            .querySelectorAll('[data-follow-uid="' + userId + '"]')
            .forEach(function (b) {
...
function switchSort(sort) {
    ...
    ["trending", "following"].forEach(function (s) {
        ...
        document
            .getElementById(idMap[s])
            ?.classList.toggle("active", s === sort);
    });
...
function setTrendingMode(mode) {
    trendingMode = mode;
    document
        .getElementById("subRelevant")
        ?.classList.toggle("active", mode === "relevant");
    document
        .getElementById("subLatest")
        ?.classList.toggle("active", mode === "latest");
    loadFeed();
}
    ...
    grid.innerHTML = all
        .map(function (t) {
            ...
            var style =
                ...
                (isActive
                    ? "var(--primary);background:var(--primary);color:white;"
                    : "var(--border);background:var(--bg-card);color:var(--text-secondary);") +
                "font-family:'DM Sans',sans-serif;font-size:11px;font-weight:600;cursor:pointer;margin-bottom:2px;";
            return (
                '<button style="' +
                style +
                '" ' +
                'onclick="sidebarTagFilter(' +
                (t.value === null ? "null" : "'" + t.value + "'") +
                ')">' +
                escH(t.label) +
                "</button>"
            );
    ...
    row.innerHTML = all
        .map(function (t) {
            return (
                ...
                (currentTagFilter === t.value ? "active" : "") +
                '" ' +
...
function renderComposerTags() {
    ...
    row.innerHTML = SUBJECT_TAGS.map(function (t) {
        return (
            '<button class="cp-tag-chip ' +
            (stagedTags.includes(t.value) ? "selected" : "") +
            '" ' +
            "onclick=\"toggleComposerTag('" +
            t.value +
            "')\">" +
            escH(t.label) +
            "</button>"
...
function toggleComposerTag(val) {
    if (stagedTags.includes(val))
        stagedTags = stagedTags.filter(function (t) {
            return t !== val;
        });
...
function renderEditTags() {
    ...
    row.innerHTML = SUBJECT_TAGS.map(function (t) {
        return (
            '<button class="cp-tag-chip ' +
            (editStagedTags.includes(t.value) ? "selected" : "") +
            '" ' +
...
function toggleEditTag(val) {
    if (editStagedTags.includes(val))
        editStagedTags = editStagedTags.filter(function (t) {
            return t !== val;
        });
...
async function loadFeed() {
    ...
    try {
        var SEL =
            "*, profiles(id, first_name, last_name, username, profile_photo_url)";
        var allMap = new Map();
        var queries = [];
        ...
        if (currentSort === "following") {
            if (!followingSet.size) {
                feed.innerHTML =
                    '<div class="feed-empty"><div class="ei">&#128065;</div><p>You\'re not following anyone yet.</p></div>';
                return;
            }
            queries.push(
                _sb
        ...
        if (!all.length) {
            feed.innerHTML =
                '<div class="feed-empty"><div class="ei">&#128205;</div><p>No posts here yet. Be the first to share something!</p></div>';
            return;
        }

        feed.innerHTML = all
            .map(function (p) {
                return postCardHTML(p, 0, 0, 0, null, 0, 0);
            })
            .join("");

        var postIds = all.map(function (p) {
            return p.id;
        });
        var sharedIds = all
            .filter(function (p) {
                return p.shared_post_id;
            })
            .map(function (p) {
                return p.shared_post_id;
            });
        ...
        (fetched[0].data || []).forEach(function (r) {
            ...
            if (r.vote === 1)
                upvoteCounts[r.post_id] = (upvoteCounts[r.post_id] || 0) + 1;
            if (r.vote === -1)
                downvoteCounts[r.post_id] =
                    (downvoteCounts[r.post_id] || 0) + 1;
        });
        (fetched[1].data || []).forEach(function (r) {
            commentCounts[r.post_id] = (commentCounts[r.post_id] || 0) + 1;
        });
        (fetched[2].data || []).forEach(function (r) {
            myVotes[r.post_id] = r.vote;
        });
        (fetched[3].data || []).forEach(function (p) {
            sharedMap[p.id] = p;
        });
        ...
        if (currentSort === "trending" && trendingMode === "relevant") {
            all.sort(function (a, b) {
                return (
                    hotScore(scores[b.id] || 0, b.created_at) -
                    hotScore(scores[a.id] || 0, a.created_at)
                );
            });
        } else {
            all.sort(function (a, b) {
        ...
        feed.innerHTML = all
            .map(function (p) {
                return postCardHTML(
                    ...
                    myVotes[p.id] || 0,
                    sharedMap[p.shared_post_id],
                    upvoteCounts[p.id] || 0,
                    downvoteCounts[p.id] || 0,
                );
            })
            .join("");
    } catch (err) {
        feed.innerHTML =
            '<div class="alert-error">Failed to load: ' +
            escH(err.message) +
            "</div>";
    }
}
...
function postCardHTML(
    ...
    var tagsHTML = "";
    if (tags.length) {
        var tagDefs = SUBJECT_TAGS.reduce(function (m, t) {
            m[t.value] = t;
            return m;
        }, {});
        tagsHTML =
    ...
    if (post.shared_post_id && sharedOriginal) {
        ...
        var ogTagsHTML = "";
        if (ogTags.length) {
            var td = SUBJECT_TAGS.reduce(function (m, t) {
                m[t.value] = t;
                return m;
            }, {});
            ogTagsHTML =
                '<div class="post-tags-row" style="margin-top:6px;">' +
                ogTags
                    .map(function (t) {
                        var d = td[t] || { label: t, cls: "tag-color-other" };
                        return (
                            '<span class="post-tag-badge ' +
                            d.cls +
                            '">' +
        ...
        var ogMediaHTML = ogMedia.length
            ? buildMediaGrid(ogMedia, og.id + "_og")
            : "";
        var ogLinkHTML = "";
        if (ogLink && ogLink.url) {
            ogLinkHTML =
                '<a class="post-link-preview" href="' +
                escH(ogLink.url) +
                '" target="_blank" rel="noopener" style="margin:8px 0 0;">' +
                (ogLink.image
                    ? '<img class="post-link-img" src="' +
                      escH(ogLink.image) +
                      '" alt="">'
                    : "") +
                '<div class="post-link-info"><div class="post-link-title">' +
                escH(ogLink.title || ogLink.url) +
                '</div><div class="post-link-url">' +
                escH(ogLink.url) +
                "</div></div></a>";
        }
        sharedHTML =
            '<div class="shared-post-embed"><div class="shared-post-header">' +
            '<a href="/profile/' +
            escH(ogA.username || og.user_id) +
            '" style="text-decoration:none;display:flex;align-items:center;gap:8px;">' +
            avatarHTML(ogA, 28) +
            '<div><div style="font-size:13px;font-weight:700;color:var(--primary);">' +
            escH(displayName(ogA)) +
            "</div>" +
            '<div style="font-size:11px;color:var(--text-light);">' +
            timeAgo(og.created_at) +
            "</div></div></a></div>" +
            ogTagsHTML +
            (og.content
                ? '<div class="post-text" style="font-size:14px;margin-top:6px;">' +
                  escH(og.content) +
                  "</div>"
                : "") +
            ogMediaHTML +
    ...
    if (files.length) {
        filesHTML =
            ...
            files
                .map(function (f) {
                    var fname =
                        f.name || (f.url || "").split("/").pop() || "File";
                    var fsize = f.size ? formatBytes(f.size) : "";
                    return (
                        '<a class="post-file-row" href="' +
                        escH(f.url) +
                        '" target="_blank" download>' +
                        '<span class="post-file-icon">' +
                        fileEmojiFromName(fname) +
                        "</span>" +
                        '<span class="post-file-name">' +
                        escH(fname) +
                        "</span>" +
                        (fsize
                            ? '<span class="post-file-size">' +
                              fsize +
                              "</span>"
                            : "") +
                        "</a>"
    ...
    if (linkMeta && linkMeta.url) {
        linkHTML =
            ...
            (linkMeta.image
                ...
                : "") +
            '<div class="post-link-info"><div class="post-link-title">' +
            escH(linkMeta.title || linkMeta.url) +
            "</div>" +
    ...
    var textHTML = text
        ? '<div class="post-text ' +
          ...
          "</div>" +
          (longText
              ? '<button class="post-see-more" onclick="togglePostText(\'' +
                post.id +
                "',this)\">See more</button>"
              : "")
        : "";

    var menuItems = isOwn
        ? '<button class="post-menu-item" onclick="openEditPost(\'' +
          post.id +
          "')\">&#9999;&#65039; Edit Post</button>" +
          '<button class="post-menu-item danger" onclick="deletePost(\'' +
          post.id +
          "')\">&#128465;&#65039; Delete Post</button>"
        : '<button class="post-menu-item" onclick="openReportModal(\'' +
          post.id +
          "')\">&#128681; Report Post</button>";
    ...
    return (
        ...
        '<a href="/profile/' +
        escH(author.username || post.user_id) +
        '" class="post-author-avatar-link" onclick="event.stopPropagation()">' +
        ...
        '<a href="/profile/' +
        escH(author.username || post.user_id) +
        '" class="post-author-name-link" onclick="event.stopPropagation()">' +
        escH(uname) +
        "</a>" +
        followBtn +
        "</div>" +
        '<div class="post-author-meta"><span>' +
        ago +
        '</span><span>&middot;</span><span class="post-vis-badge" title="' +
        post.visibility +
        '">' +
        visIcon +
        "</span></div>" +
        ...
        linkHTML +
        (sharedHTML
            ? '<div style="padding:0 14px 10px;">' + sharedHTML + "</div>"
            : "") +
        '<div class="post-action-bar-b">' +
        '<div class="vote-group-b">' +
        '<button class="vote-btn-b ' +
        (upvoted ? "upvoted-b" : "") +
        '" id="upBtn-' +
        post.id +
        '" onclick="castVote(\'' +
        ...
        (upvoted ? "currentColor" : "none") +
        '" stroke="currentColor" stroke-width="2.5" style="width:14px;height:14px;"><polyline points="18 15 12 9 6 15"/></svg>' +
        '<span class="vote-count upvote-count" id="upCount-' +
        post.id +
        '">' +
        (upvoteCount || 0) +
        "</span>" +
        ...
        '\',-1)" title="Downvote">' +
        '<svg viewBox="0 0 24 24" fill="' +
        (downvoted ? "currentColor" : "none") +
        '" stroke="currentColor" stroke-width="2.5" style="width:14px;height:14px;"><polyline points="6 9 12 15 18 9"/></svg>' +
        '<span class="vote-count downvote-count" id="downCount-' +
        post.id +
        '">' +
        (downvoteCount || 0) +
        "</span>" +
        ...
        '<span id="score-' +
        post.id +
        '" style="display:none;" class="vote-score ' +
        scoreClass +
        '">' +
        score +
        "</span>" +
        ...
        '<button class="post-action-btn-b" onclick="openComments(\'' +
        post.id +
        "')\">" +
        ...
        "</button>" +
        '<button class="post-action-btn-b" onclick="openShareModal(\'' +
        post.id +
        "')\">" +
...
async function castVote(postId, value) {
    ...
    var newVote =
        (value === 1 && prevUp) || (value === -1 && prevDown) ? 0 : value;
    ...
    if (scoreEl) {
        ...
        scoreEl.className =
            "vote-score " + (ns > 0 ? "positive" : ns < 0 ? "negative" : "");
    }
    ...
    if (upBtn)
        upBtn
            .querySelector("svg")
            .setAttribute("fill", newVote === 1 ? "currentColor" : "none");
    if (downBtn)
        downBtn
            .querySelector("svg")
            .setAttribute("fill", newVote === -1 ? "currentColor" : "none");
    ...
    try {
        ...
        if (upBtn)
            upBtn
                .querySelector("svg")
                .setAttribute("fill", prevVote === 1 ? "currentColor" : "none");
        if (downBtn)
            downBtn
                .querySelector("svg")
                .setAttribute(
                    "fill",
                    prevVote === -1 ? "currentColor" : "none",
                );
    }
}
...
function buildMediaGrid(media, postId) {
    ...
    var imageUrls = media.filter(function (u) {
        return !/\.(mp4|mov|webm|ogg)(\?|$)/i.test(u);
    });
    var imgCounter = 0;
    return (
        ...
        shown
            .map(function (url, i) {
                ...
                var idx = imgCounter++;
                var b64 = btoa(
                    unescape(encodeURIComponent(JSON.stringify(imageUrls))),
                );
                return (
                    '<div class="post-media-item" style="cursor:zoom-in;" onclick="openLightboxB64(\'' +
                    b64 +
                    "'," +
                    idx +
                    ')">' +
...
function openLightboxB64(b64, startIndex) {
    try {
        lightboxImages = JSON.parse(decodeURIComponent(escape(atob(b64))));
    } catch (e) {
        console.error("lightbox decode", e);
        return;
    }
...
function closeComments(e, force) {
    if (!force && e && e.target !== document.getElementById("commentsOverlay"))
        return;
...
async function loadComments(postId) {
    ...
    try {
        ...
        var commentIds = (r.data || []).map(function (c) {
            return c.id;
        });
        var repliesMap = {};
        ...
        var total =
            (r.data?.length || 0) +
            Object.values(repliesMap).reduce(function (s, a) {
                return s + a.length;
            }, 0);
        updateCommentCount(postId, total);
        if (!r.data?.length) {
            list.innerHTML =
                '<div class="res-loading-sm">No comments yet!</div>';
            return;
        }
        list.innerHTML = r.data
            .map(function (c) {
                return commentHTML(c, repliesMap[c.id] || []);
            })
            .join("");
    } catch (e) {
...
function commentHTML(c, replies) {
    ...
    return (
        ...
        escH(name) +
        "</div>" +
        '<div class="comment-text" id="ctext-' +
        c.id +
        '">' +
        escH(c.content) +
        "</div>" +
        ...
        "</span>" +
        '<button class="comment-action-btn" onclick="startReply(\'' +
        c.id +
        "','" +
        escH(name) +
        "')\">Reply</button>" +
        (isOwn
            ? '<button class="comment-action-btn" onclick="startEditComment(\'' +
              c.id +
              "')\">Edit</button>" +
              '<button class="comment-action-btn danger" onclick="deleteComment(\'' +
              c.id +
              "')\">Delete</button>"
            : "") +
        "</div>" +
        ...
        '<div id="replyBox-' +
        c.id +
        '" style="display:none;" class="reply-input-wrap">' +
        ...
        c.id +
        "');}\"></textarea>" +
        '<button class="reply-send-btn" onclick="submitReply(\'' +
        c.id +
        "')\">Send</button>" +
        "</div>" +
        "</div>" +
...
function startReply(commentId, authorName) {
    document.querySelectorAll('[id^="replyBox-"]').forEach(function (b) {
...
async function submitReply(parentId) {
    ...
    try {
        var r = await _sb.from("post_comments").insert({
...
function startEditComment(commentId) {
    ...
    textEl.innerHTML =
        '<textarea class="reply-textarea" style="width:100%;min-height:40px;" id="cedit-' +
        commentId +
        '">' +
        escH(original) +
        "</textarea>" +
        '<div style="display:flex;gap:6px;margin-top:6px;justify-content:flex-end;">' +
        '<button class="comment-action-btn" onclick="cancelEditComment(\'' +
        commentId +
        "','" +
        escH(original) +
        "')\">Cancel</button>" +
        '<button class="reply-send-btn" style="padding:5px 12px;font-size:12px;" onclick="saveEditComment(\'' +
        commentId +
        "')\">Save</button></div>";
}
...
function toggleLinkInput() {
    ...
    if (row.style.display !== "none")
        document.getElementById("cpLinkInput")?.focus();
}
...
function renderLinkPreview() {
    ...
    el.innerHTML =
        ...
        (stagedLink.image
            ? '<img class="cp-link-preview-thumb" src="' +
              escH(stagedLink.image) +
              '" alt="">'
            : "") +
        '<div class="cp-link-preview-info"><div class="cp-link-preview-title">' +
        escH(stagedLink.title || stagedLink.url) +
        "</div>" +
        '<div class="cp-link-preview-url">' +
        escH(stagedLink.url) +
        "</div></div>" +
...
function handleMediaFiles(fileList) {
    Array.from(fileList).forEach(function (file) {
        ...
        if (
            !stagedMedia.find(function (m) {
                return m.file.name === file.name && m.file.size === file.size;
            })
        )
    ...
    grid.innerHTML = stagedMedia
        .map(function (m, i) {
            return (
                ...
                '<button class="cp-remove-media" onclick="removeMedia(' +
                i +
                ')">&#10005;</button></div>'
            );
        })
...
function renderFileChips() {
    ...
    if (!chips) {
        ...
        document
            .getElementById("cpMediaPreview")
            .insertAdjacentElement("afterend", chips);
    }
    chips.innerHTML = stagedFiles
        .map(function (f, i) {
            return (
                '<div class="cp-file-chip">' +
                fileEmojiFromName(f.name) +
                " " +
                escH(f.name) +
    ...
    try {
        ...
        for (var m of stagedMedia) {
            ...
            var path =
                ...
                "." +
                ext;
            var u = await _sb.storage
                .from("posts")
                .upload(path, m.file, { upsert: true });
            if (u.error) throw u.error;
            mediaUrls.push(
                _sb.storage.from("posts").getPublicUrl(path).data.publicUrl,
            );
        }
        for (var f of stagedFiles) {
            ...
            if (u.error) throw u.error;
            fileUrls.push({
                url: _sb.storage.from("posts").getPublicUrl(path).data
                    .publicUrl,
                name: f.name,
                size: f.size,
            });
        ...
        var r = await _sb.from("posts").insert({
            ...
            tags: stagedTags.length ? stagedTags : null,
        });
        if (r.error) throw r.error;
        cancelComposer();
        loadFeed();
...
async function openEditPost(postId) {
    ...
    editStagedTags = parseJSON(post?.tags, []);
    document.getElementById("editContent").value = post?.content || "";
    document.getElementById("editVisSelect").value =
        post?.visibility || "public";
    renderEditMediaPreview();
    renderEditFileChips();
    renderEditLinkPreview();
    renderEditTags();
    var lr = document.getElementById("editLinkRow");
    if (lr) lr.style.display = "none";
    document.getElementById("editModal").classList.add("open");
    document.getElementById("postMenu-" + postId)?.classList.remove("open");
}
function renderEditMediaPreview() {
    var grid = document.getElementById("editMediaPreview");
    if (!grid) return;
    var items = [
        ...editExistingMedia.map(function (url, i) {
            return {
                type: "existing",
                url,
                i,
        ...
        ...editStagedMedia.map(function (m, i) {
            return {
                type: "new",
                url: m.objectUrl,
                i,
    ...
    grid.innerHTML = items
        .map(function (item) {
            return (
                ...
                '<button class="cp-remove-media" onclick="' +
                (item.type === "existing"
                    ? "removeEditExistingMedia(" + item.i + ")"
                    : "removeEditNewMedia(" + item.i + ")") +
                '">&#10005;</button></div>'
            );
        })
        .join("");
}
function removeEditExistingMedia(i) {
    editExistingMedia.splice(i, 1);
    renderEditMediaPreview();
}
...
function renderEditFileChips() {
    var wrap = document.getElementById("editFileChips");
    if (!wrap) return;
    var ex = editExistingFiles
        .map(function (f, i) {
            return (
                '<div class="cp-file-chip">' +
                fileEmojiFromName(f.name || "") +
                " " +
                escH(f.name || f.url?.split("/").pop() || "File") +
                '<button onclick="removeEditExistingFile(' +
                i +
                ')">&#10005;</button></div>'
            );
        })
        .join("");
    var nf = editStagedFiles
        .map(function (f, i) {
            return (
                ...
                " " +
                escH(f.name) +
                ' <small>(new)</small><button onclick="removeEditNewFile(' +
                i +
                ')">&#10005;</button></div>'
            );
        })
...
function removeEditExistingFile(i) {
    editExistingFiles.splice(i, 1);
    renderEditFileChips();
}
function handleEditFileAttach(fileList) {
    Array.from(fileList).forEach(function (f) {
        editStagedFiles.push({ file: f, name: f.name, size: f.size });
    });
    renderEditFileChips();
}
...
function toggleEditLinkInput() {
    var row = document.getElementById("editLinkRow");
    if (!row) return;
    row.style.display = row.style.display === "none" ? "" : "none";
    if (row.style.display !== "none") {
        var inp = document.getElementById("editLinkInput");
        if (inp) {
            inp.value = editStagedLink?.url || "";
            inp.focus();
        }
    }
}
async function fetchEditLinkPreview(url) {
    ...
    try {
        ...
        console.warn("[EditOG]", e.message);
    }
}
function renderEditLinkPreview() {
    var el = document.getElementById("editLinkPreview");
    if (!el) return;
    if (!editStagedLink) {
    ...
    el.innerHTML =
        '<div class="cp-link-preview-inner">' +
        (editStagedLink.image
            ? '<img class="cp-link-preview-thumb" src="' +
              escH(editStagedLink.image) +
              '" alt="">'
            : "") +
        '<div class="cp-link-preview-info"><div class="cp-link-preview-title">' +
        escH(editStagedLink.title || editStagedLink.url) +
        "</div>" +
        '<div class="cp-link-preview-url">' +
        escH(editStagedLink.url) +
        "</div></div>" +
...
function closeEditModal() {
    ...
    editingPostId = null;
    editStagedMedia.forEach(function (m) {
    ...
    try {
        ...
        var r = await _sb
            ...
            .update({
                ...
                visibility: vis,
                media_urls: fm.length ? fm : null,
                file_urls: ff.length ? ff : null,
...
async function deletePost(postId) {
    ...
    try {
        var r = await _sb.from("posts").delete().eq("id", postId);
        if (r.error) throw r.error;
...
function openReportModal(postId) {
    ...
    document
        .querySelectorAll('input[name="postReportReason"]')
        .forEach(function (r) {
            r.checked = false;
        });
...
async function submitPostReport() {
    var reason = document.querySelector(
        'input[name="postReportReason"]:checked',
    );
    if (!reason) {
    ...
    try {
        var r = await _sb.from("reports").insert({
...
async function shareToFeed() {
    ...
    try {
        ...
        if (r.error) throw r.error;
        document.getElementById("shareConfirmMsg").textContent = "Shared!";
...
async function loadWhoToFollow() {
    ...
    try {
        var r = await _sb
            ...
            .limit(20);
        var suggestions = (r.data || [])
            .filter(function (u) {
                return !followingSet.has(u.id);
            })
            .slice(0, 5);
        if (!suggestions.length) {
            el.innerHTML =
                '<div class="res-empty-small">Nothing to suggest.</div>';
            return;
        }
        el.innerHTML = suggestions
            .map(function (u) {
                ...
                return (
                    ...
                    '<a href="' +
                    profileUrl +
                    '" class="wtf-avatar" style="text-decoration:none;color:inherit;">' +
                    (u.profile_photo_url
                        ? '<img src="' +
                          escH(u.profile_photo_url) +
                          '" alt="' +
                          escH(uname) +
                    ...
                    '<a href="' +
                    profileUrl +
                    '" class="wtf-info-link"><div class="wtf-name">' +
                    escH(uname) +
                    '</div><div class="wtf-sub">' +
                    escH(full) +
                    "</div></a>" +
                    '<button class="wtf-follow-btn ' +
                    (followingSet.has(u.id) ? "following" : "") +
                    '" data-follow-uid="' +
                    u.id +
                    '" onclick="event.preventDefault();toggleFollow(\'' +
                    u.id +
                    "',this)\">" +
...
async function loadTrending() {
    ...
    try {
        ...
        if (!sorted.length) {
            el.innerHTML =
                '<div class="res-empty-small">No trending topics yet.</div>';
            return;
        }
        el.innerHTML = sorted
            .map(function (entry, i) {
                return (
                    '<div class="trending-item" onclick="searchByTag(\'' +
                    escH(entry[0]) +
                    '\')"><span class="trending-rank">' +
                    (i + 1) +
                    '</span><span class="trending-tag">' +
                    escH(entry[0]) +
                    '</span><span class="trending-count">' +
                    entry[1] +
                    " post" +
                    (entry[1] !== 1 ? "s" : "") +
                    "</span></div>"
...
function searchByTag(tag) {
    ...
    var match = SUBJECT_TAGS.find(function (t) {
        return t.value === clean;
    });
    if (match) {
        setTagFilter(match.value);
...
async function loadTrendingResources() {
    ...
    if (!el) return;
    try {
        var res = await fetch("/api/resources/trending?limit=5", {
        ...
        if (!data.length) {
            el.innerHTML =
                '<div class="res-empty-small">No rated resources yet.</div>';
            return;
        }
        el.innerHTML = data
            .map(function (r) {
                var stars = starsHTML(parseFloat(r.avg_rating) || 0);
                var rating = r.avg_rating
                ...
                var subj = r.subject || "";
                return (
                    '<a href="/resources?open=' +
                    escH(r.id) +
                    '" style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);text-decoration:none;">' +
                    '<div style="width:32px;height:32px;border-radius:8px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:16px;background:var(--bg-main);">' +
                    icon +
                    "</div>" +
                    '<div style="flex:1;min-width:0;">' +
                    '<div style="font-size:12px;font-weight:600;color:var(--text-primary);line-height:1.4;margin-bottom:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' +
                    escH(r.title) +
                    '">' +
                    escH(r.title) +
                    "</div>" +
        ...
        el.innerHTML =
            '<div class="res-empty-small">Could not load resources.</div>';
        console.warn("[trendingResources]", e.message);
    }
}
...
async function loadActiveStudyGroups() {
    ...
    if (!el) return;
    try {
        var res = await fetch("/api/study-groups/active?limit=4", {
            headers: { "X-CSRF-TOKEN": window.CSRF_TOKEN || "" },
        });
        if (!res.ok) throw new Error("HTTP " + res.status);
        var json = await res.json();
        ...
        if (!data.length) {
            el.innerHTML =
                '<div class="res-empty-small">No active groups yet.</div>';
            return;
        }
        el.innerHTML = data
            .map(function (g) {
                var initials = (g.name || "?").slice(0, 2).toUpperCase();
                var memberCount = g.member_count
                    ? g.member_count +
                      " member" +
                      (g.member_count != 1 ? "s" : "")
                    : "";
                ...
                var isActive =
                    g.last_message_at &&
                    Date.now() - new Date(g.last_message_at) < 172800000;
                return (
                    '<a href="/study-groups" style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);text-decoration:none;">' +
                    '<div style="width:32px;height:32px;border-radius:10px;flex-shrink:0;background:linear-gradient(135deg,var(--primary),var(--primary-dark));display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:white;">' +
                    escH(initials) +
                    "</div>" +
                    '<div style="flex:1;min-width:0;">' +
                    '<div style="font-size:12px;font-weight:600;color:var(--text-primary);line-height:1.4;margin-bottom:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' +
                    escH(g.name) +
                    '">' +
                    escH(g.name) +
                    "</div>" +
                    ...
                    "</div>" +
                    (isActive
                        ? '<div style="width:7px;height:7px;border-radius:50%;background:#22c55e;flex-shrink:0;" title="Active in last 48h"></div>'
                        : "") +
                    "</a>"
                );
            })
        ...
        el.innerHTML =
            '<div class="res-empty-small">Could not load groups.</div>';
        console.warn("[activeGroups]", e.message);
    }
}
...
document.addEventListener("keydown", function (e) {
    ...
    if (document.getElementById("commentsOverlay").classList.contains("open")) {
        closeComments(null, true);
        return;
    }
