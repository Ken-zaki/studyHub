{{-- studyhub/resources/views/admin/sidebar.blade.php --}}
@php
...
<aside class="adm-sidebar">
    ...
    <nav class="adm-nav">
        ...
        <a href="{{ route('admin.dashboard') }}"
            class="adm-nav-item {{ $activeAdmin === 'dashboard' ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.resources') }}"
            class="adm-nav-item {{ $activeAdmin === 'resources' ? 'active' : '' }}">
            <svg class="adm-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <a href="{{ route('admin.announcements') }}"
            class="adm-nav-item {{ $activeAdmin === 'announcements' ? 'active' : '' }}">
            <svg class="adm-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <a href="{{ route('admin.settings') }}"
            class="adm-nav-item {{ $activeAdmin === 'settings' ? 'active' : '' }}">
            <svg class="adm-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <circle cx="12" cy="12" r="3" />
                <path
                    d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-2 2 2 2 0 01-2-2v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06A1.65 1.65 0 004.6 9a1.65 1.65 0 00-1.51-1H3a2 2 0 01-2-2 2 2 0 012-2h.09A1.65 1.65 0 004.6 9" />
            </svg>
            Settings
        </a>
    ...
    async function loadSidebarBadges() {
        ...
        try {
            ...
            const [rRes, resRes] = await Promise.all([
                fetch(`${window.SB_URL}/rest/v1/reports?status=eq.pending&select=id`, {
                    headers
                }),
                fetch(`${window.SB_URL}/rest/v1/resources?is_approved=eq.false&select=id`, {
                    headers
                }),
            ]);
            ...
            if (rb) {
                rb.textContent = rCount;
                rb.style.display = rCount ? '' : 'none';
            }
            if (ab) {
                ab.textContent = resCount;
                ab.style.display = resCount ? '' : 'none';
            }
        } catch (e) {}
    }
    loadSidebarBadges();
