        ...
        <div class="profile-column">
            ...
            @if ($errors->any())
                <div class="friend-status-banner"
                    style="margin-bottom:16px;background:#fef2f2;color:#991b1b;border-color:#fecaca;">
                    {{ $errors->first() }}</div>
            @endif
                            ...
                            @if (($userId ?? '') !== session('user_id'))
                                @if (($relationshipState ?? 'none') === 'friends')
                                    <button type="button" class="profile-upload-btn profile-add-friend-btn"
                                        disabled>Friends</button>
                                    <form method="POST"
                                        action="{{ route('friends.remove', ['friendId' => $userId ?? '']) }}">
                                        @csrf
                                ...
                                @elseif(($relationshipState ?? 'none') === 'pending_incoming' && !empty($pendingRequestId))
                                    <form method="POST"
                                        action="{{ route('friend-requests.accept', ['friendRequest' => $pendingRequestId]) }}">
                                        @csrf
                                        <button type="submit"
                                            class="profile-upload-btn profile-add-friend-btn">Accept</button>
                                    </form>
                                    <form method="POST"
                                        action="{{ route('friend-requests.decline', ['friendRequest' => $pendingRequestId]) }}">
                                        @csrf
                                        <button type="submit" class="profile-upload-btn"
                                ...
                                @else
                                    <form method="POST"
                                        action="{{ route('friend-requests.send', ['receiverId' => $userId ?? '']) }}">
                                        @csrf
                                        <button type="submit" class="profile-upload-btn profile-add-friend-btn">Add
                                            Friend</button>
                                    </form>
                                @endif
                                    ...
                                    @foreach ($friendList as $friend)
                                        @php
                                            ...
                                            if ($friendInitials === '') {
                                                ...
                                                $friendInitials = strtoupper(
                                                    substr((string) ($parts[0] ?? 'F'), 0, 1) .
