...
<head>
    ...
    <style>
        ...
        .friends-list::-webkit-scrollbar-thumb { background: #ddd9d2; border-radius: 4px; }
        ...
        .friend-item:hover { background: #faf9f7; }
        .friend-item.active { background: #f0ece4; }
        ...
        .no-friends span { font-size: 2rem; display: block; margin-bottom: 10px; }
        ...
        .messages-area::-webkit-scrollbar-thumb { background: #ddd9d2; border-radius: 4px; }
        ...
        .msg-row.group-end .msg-bubble { border-radius: 18px; }
        .msg-row.received.group-end .msg-bubble { border-bottom-left-radius: 4px; }
        .msg-row.sent.group-end .msg-bubble { border-bottom-right-radius: 4px; }
        ...
        .chat-input-wrapper:focus-within { border-color: #8b7355; }
        ...
        #messageInput::placeholder { color: #bbb; }
        ...
        .send-btn:hover { background: #7a6347; }
...
<main class="main-content-simple" style="padding: 0; overflow: hidden;">
    <div class="messages-layout">
                ...
                @forelse($friends as $friend)
                    @php
                        ...
                        if ($lastMsg) {
                            ...
                            $timeStr = $ts->isToday()
                                ? $ts->format('g:i A')
                                : ($ts->isYesterday() ? 'Yesterday' : $ts->format('M j'));
                        }
                    @endphp
                    <div class="friend-item"
                        ...
                        <div class="friend-avatar">
                            @if($friend->profile_photo_url)
                                <img src="{{ $friend->profile_photo_url }}" alt="{{ $friend->first_name }}" onerror="this.style.display='none'; this.parentElement.textContent='{{ $initials }}'">
                            @else
                                {{ $initials }}
                            @endif
                        </div>
                        ...
                        <div class="friend-meta">
                            ...
                            @if($friend->unread_count > 0)
                                <span class="unread-badge" id="badge-{{ $friend->id }}">{{ $friend->unread_count }}</span>
                            @else
                                <span class="unread-badge" id="badge-{{ $friend->id }}" style="display:none;">0</span>
                            @endif
                        </div>
                    </div>
                @empty
        ...
        <section class="chat-panel" id="chatPanel">
            ...
            <div id="chatActive" style="display:none; flex-direction:column; flex:1; overflow:hidden; display:none;">
