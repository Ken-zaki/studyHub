...
(function () {
    ...
    el.flashcardCancelBtn?.addEventListener('click', () => { resetFlashcardForm(); setFlashcardStatus(''); });
    ...
    function renderFlashcards() {
        ...
        if (!state.flashcards.length) {
            el.flashcardList.innerHTML = '<div class="flashcard-empty">No flashcards yet. Click "Create Flashcard" to add one.</div>';
            return;
        }
        el.flashcardList.innerHTML = state.flashcards.map((c, i) => `
    ...
    async function handleFlashcardSubmit(e) {
        ...
        if (!question || !answer) { setFlashcardStatus('Please fill in both fields.', true); return; }
        try {
            ...
            state.flashcards = payload.flashcards || [...state.flashcards, payload.flashcard];
            renderFlashcards();
    ...
    function onPhaseComplete() {
        pausePomodoro();
        playChime();
        showNotif(state.pomoPhase === 'focus'
    ...
    function updateRoundLabel() {
        ...
        if (el) el.textContent = Math.min((state.pomoCycle % POMO.cyclesBeforeLong) + 1, POMO.cyclesBeforeLong);
    }
