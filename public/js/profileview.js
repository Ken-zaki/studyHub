...
const viewedUsername =
    document.querySelector('meta[name="data-viewed-username"]')?.content || "";
const viewedUserIdMeta =
    document.querySelector('meta[name="data-viewed-user-id"]')?.content || "";
const currentUserId =
    document.querySelector('meta[name="data-current-user-id"]')?.content || "";
...
async function resolveViewedUser() {
    ...
    try {
        const res = await sbGet(
            `/rest/v1/profiles?username=eq.${encodeURIComponent(slug)}&select=id`,
        );
...
async function loadSelfUI() {
    ...
    try {
        const res = await sbGet(
            `/rest/v1/profiles?id=eq.${currentUserId}&select=first_name,last_name,profile_photo_url`,
        );
...
async function loadViewedUserInfo() {
    if (!viewedUserId) {
        setText("profileFullName", "User not found");
        return;
    }
    try {
        const res = await sbGet(
        ...
        setAvatar(
            ...
            p.profile_photo_url,
            name,
            mkInitials(p.first_name, p.last_name),
        );
...
async function loadViewedUserStats() {
    ...
    try {
        const res = await sbGet(
            `/rest/v1/posts?user_id=eq.${viewedUserId}&visibility=eq.public&select=id`,
        );
        const data = await res.json();
...
async function loadViewedUserPosts() {
    ...
    if (!feed || !viewedUserId) {
        if (feed)
            feed.innerHTML = `<div class="error-state">❌ Could not determine user.</div>`;
        return;
    }
    try {
        const res = await sbGet(
    ...
    if (files.length) {
        mediaHTML +=
            ...
            files
                .map(
                    (f) =>
                        `<a class="post-file-chip" href="${escH(f.url)}" target="_blank" download>📎 <span>${escH(f.name || "File")}</span></a>`,
                )
                .join("") +
            `</div>`;
    }
