...
(function () {
    ...
    function updateFocusTopBackButtonVisibility() {
        ...
        const shouldShow = state.currentScreen !== "screenMenu";
        el.topBarFocusBackBtn.classList.toggle("hidden", !shouldShow);
        el.topBarFocusBackBtn.setAttribute("aria-hidden", String(!shouldShow));
    }

    function goToFocusMenu() {
        showScreen("screenMenu");
    }

    el.topBarFocusBackBtn?.addEventListener("click", goToFocusMenu);
    updateFocusTopBackButtonVisibility();
    ...
    bgAudio.src = "/music/relaxing-music.mp3";
    bgAudio.loop    = true;
   ...
   el.musicToggleBtn.addEventListener("click", () => {
    ...
    if (state.musicOn && bgAudio.paused) {
        startMusic();
    }
    updateMusicFabVisibility();
});
