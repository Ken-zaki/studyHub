...
(function () {
    "use strict";
    console.error('🔴 FOCUS-MODE.JS IIFE STARTED');
    ...
    const state = {
        ...
        flashcards: Array.isArray(window.__focusFlashcards)
            ...
            : [],
        quizzes: Array.isArray(window.__focusQuizzes)
            ? window.__focusQuizzes
            : [],
        decks: Array.isArray(window.__focusDecks)
            ? window.__focusDecks
            : [],
        currentDeckId: null,
        uploadBusy: false,
    ...
    const el = {
        ...
        deckStudyView: $("deckStudyView"),
        deckCreateBtn: $("deckCreateBtn"),
        deckForm: $("deckCreateForm"),
        deckName: $("deckNameInput"),
        deckDesc: $("deckDescInput"),
        deckCancelBtn: $("deckCancelBtn"),
        deckSaveBtn: $("deckSaveBtn"),
        decksGrid: $("deckGrid"),
        deckStatus: $("deckStatus"),
        flashcardBackBtn: $("flashcardBackBtn"),
        flashcardMenuBtn: $("flashcardMenuBtn"),
    ...
    function showScreen(id) {
        ...
        // Special handling for flashcard screen: show deck selection by default
    ...
    async function handleMaterialUpload(e) {
        ...
        try {
            ...
            console.error("Focus mode material upload failed:", err);
        } finally {
    ...
    el.deckCreateBtn?.addEventListener("click", () => {
        if (!el.deckForm) return;
        // show as floating modal
        el.deckForm.classList.remove("hidden");
        el.deckForm.classList.add("floating");
        el.deckCreateBtn.classList.add("hidden");
        // add backdrop
        const bd = document.createElement("div");
        bd.id = "deckCreateBackdrop";
        bd.className = "deck-create-backdrop";
        bd.addEventListener("click", () => resetDeckForm());
    ...
    el.deckForm?.addEventListener("submit", handleDeckSubmit);
    el.flashcardBackBtn?.addEventListener("click", () => {
    ...
    function resetDeckForm() {
        if (!el.deckForm) return;
        el.deckForm?.reset();
        el.deckForm?.classList.remove("floating");
        el.deckForm?.classList.add("hidden");
        el.deckCreateBtn?.classList.remove("hidden");
        const bd = document.getElementById("deckCreateBackdrop");
        if (bd) bd.remove();
    }

    function renderDecks() {
        if (!el.decksGrid) return;
        if (!state.decks.length) {
            el.decksGrid.innerHTML =
                '<div class="decks-empty">No decks yet. Click "New Deck" to create one.</div>';
            return;
        }
        el.decksGrid.innerHTML = state.decks
            .map(
                (d) => `
            <button class="deck-card" data-deck-id="${escHtml(d.id)}" type="button">
                <div class="deck-card-icon">📚</div>
                <div class="deck-card-name">${escHtml(d.name)}</div>
                <div class="deck-card-count">${d.cardCount} card${d.cardCount !== 1 ? 's' : ''}</div>
            </button>`,
            )
            .join("");
    ...
    function selectDeck(deckId) {
        ...
        state.flashcards = state.flashcards.filter((fc) => fc.deck_id === deckId);
        showDeckStudyView();
        renderFlashcards();
    }

    function showDeckSelectionView() {
        if (el.deckSelectionView && el.deckStudyView) {
            el.deckSelectionView.classList.remove("hidden");
            el.deckStudyView.classList.add("hidden");
            el.flashcardMenuBtn?.classList.remove("hidden");
        }
        renderDecks();
    }

    function showDeckStudyView() {
        if (el.deckSelectionView && el.deckStudyView) {
            el.deckSelectionView.classList.add("hidden");
            el.deckStudyView.classList.remove("hidden");
            el.flashcardMenuBtn?.classList.add("hidden");
        }
    }
    ...
    async function handleDeckSubmit(e) {
        ...
        if (!name) {
            setDeckStatus("Please enter a deck name.", true);
            return;
        }
        try {
            state.deckBusy = true;
            if (el.deckSaveBtn) el.deckSaveBtn.disabled = true;
            setDeckStatus("Creating deck…");
            ...
            state.decks = payload.decks || [...state.decks, payload.deck];
            renderDecks();
            setDeckStatus("Deck created!");
            resetDeckForm();
        } catch (err) {
            setDeckStatus(err.message || "Create failed.", true);
        } finally {
    ...
    /* ── Quiz Questions ───────────────────────────────────── */
    el.quizCreateBtn?.addEventListener("click", () => {
        el.quizForm.classList.remove("hidden");
        el.quizCreateBtn.classList.add("hidden");
        el.quizQuestion?.focus();
    });
    el.quizCancelBtn?.addEventListener("click", () => {
        resetQuizForm();
        setQuizStatus("");
    });
    ...
    async function handleFlashcardSubmit(e) {
        ...
        try {
            ...
            const res = await fetch("/focus-mode/flashcards", {
                ...
                body: JSON.stringify({ question, answer, deck_id: state.currentDeckId }),
            });
            const payload = await res.json();
    ...
    function resetQuizForm() {
        el.quizForm?.reset();
        el.quizForm?.classList.add("hidden");
        el.quizCreateBtn?.classList.remove("hidden");
    }
    function renderQuizzes() {
        if (!el.quizList) return;
        if (!state.quizzes.length) {
            el.quizList.innerHTML =
                '<div class="quiz-empty">No quiz questions yet. Click "Create Quiz Question" to add one.</div>';
            return;
        }
        el.quizList.innerHTML = state.quizzes
            .map(
                (q, i) => `
            <article class="typed-quiz-item">
                <div class="typed-quiz-index">Question ${i + 1}</div>
                <div class="typed-quiz-question">${escHtml(q.question)}</div>
                <div class="typed-quiz-options">
                    ${Object.entries(q.options || {})
                        .map(([key, value]) => {
                            const isCorrect = key === q.correct_option;
                            return `
                                <div class="typed-quiz-option ${isCorrect ? 'correct' : ''}">
                                    <span class="typed-quiz-option-key">${escHtml(key)}</span>
                                    <span class="typed-quiz-option-value">${escHtml(value)}</span>
                                    ${isCorrect ? '<span class="typed-quiz-correct-badge">Correct</span>' : ''}
                                </div>`;
                        })
                        .join('')}
                </div>
                ${q.explanation ? `<div class="typed-quiz-explanation">${escHtml(q.explanation)}</div>` : ''}
            </article>`,
            )
            .join("");
    }
    ...
    showScreen("screenMenu");
            ...
            console.error('🔴 FOCUS-MODE.JS ERROR:', error.message, error.stack);
            window.__focusModeError = error;
        }
    renderFlashcards();
    renderQuizzes();
    renderDecks();
