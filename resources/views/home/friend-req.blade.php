...
<body>
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('friend-requests') }}" class="nav-item active">
                <svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M16 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/>
                    <circle cx="12" cy="7" r="4"/>
                    <path d="M22 16.92v3m0 0v3m0-3h3m0 0h3"/>
                </svg>
                <span class="nav-text">Friend Requests</span>
                <span class="nav-badge">{{ $incomingRequests->count() }}</span>
            </a>
    ...
    <main class="main-content">
        <div class="requests-shell">
            ...
            <div class="content-layout">
                <!-- Incoming Requests -->
                <section class="panel">
                    <div class="panel-title">Incoming Requests ({{ $incomingRequests->count() }})</div>
                    <div class="requests-list">
                            ...
                            <div class="request-item">
                                <div class="request-info">
                                    <div class="request-avatar">
                                        @php
                                            $senderFirstName = trim((string) ($request->sender->first_name ?? ''));
                                            $senderLastName = trim((string) ($request->sender->last_name ?? ''));
                                            $senderName = trim($senderFirstName.' '.$senderLastName);
                                            if ($senderName === '') {
                                                $senderName = trim((string) ($request->sender->name ?? $request->sender->username ?? 'U'));
                                            }
                                            $parts = preg_split('/\s+/', $senderName, -1, PREG_SPLIT_NO_EMPTY) ?: [];
                                ...
                                <div class="request-actions">
                                    ...
                                    <form method="POST" action="{{ route('friend-request.reject', $request->id) }}" style="display: inline;">
                                        @csrf
                        ...
                        @empty
                            <p class="muted-note">No incoming friend requests.</p>
                        @endforelse
                    </div>
                </section>
                ...
                <!-- Sent Requests -->
                <section class="panel">
                    <div class="panel-title">Sent Requests ({{ $sentRequests->count() }})</div>
                    <div class="requests-list">
                        @forelse ($sentRequests as $request)
                            <div class="request-item">
                                <div class="request-info">
                                    <div class="request-avatar">
                                        @php
                                            $receiverFirstName = trim((string) ($request->receiver->first_name ?? ''));
                                            $receiverLastName = trim((string) ($request->receiver->last_name ?? ''));
                                            $receiverName = trim($receiverFirstName.' '.$receiverLastName);
                                            if ($receiverName === '') {
                                                $receiverName = trim((string) ($request->receiver->name ?? $request->receiver->username ?? 'U'));
                                            }
                                            $parts = preg_split('/\s+/', $receiverName, -1, PREG_SPLIT_NO_EMPTY) ?: [];
                                            $initials = strtoupper(substr($parts[0] ?? 'U', 0, 1).substr($parts[1] ?? '', 0, 1));
                                        @endphp
                                        {{ $initials }}
                                    </div>
                                    <div class="request-meta">
                                        <div class="request-name">{{ $request->receiver->name }}</div>
                                        <div class="request-time">{{ $request->created_at?->diffForHumans() }}</div>
                                    </div>
                                </div>
                                <div class="request-actions">
                                    <span class="btn btn-warning" style="cursor: default;"><small>Pending</small></span>
                                </div>
                            </div>
                        @empty
                            <p class="muted-note">No sent friend requests.</p>
                        @endforelse
                    </div>
                </section>
            </div>
